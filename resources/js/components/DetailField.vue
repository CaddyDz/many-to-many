	...
	<panel-item :field="field">
		<template slot="value" v-if="field.value && field.value.length">
			<span v-if="field.pivots">
				...
				>{{ resource.text }} {{ field.value.length - index - 1 ? ' , ' : '' }}</span>
			</span>
			<span v-else>
				<router-link
					v-for="(resource, index) in field.value"
            ...
            params: {
              ...
              resourceId: resource.id,
					...
					:title="__('View')"
				>{{ resource.text }} {{ field.value.length - index - 1 ? ' , ' : '' }}</router-link>
			</span>
						...
						<div class="px-6 py-3 flex">
							<div class="flex items-center ml-auto">
								...
								<router-link
									...
									class="no-underline font-bold dim text-primary"
									:title="__('View')"
								>{{ __(':resource Details', { resource: display.text }) }}</router-link>
							</div>
						</div>
					</card>
				</loading-view>
